...
saveRDS(t_full_fit_finished, file = "01_Full_Set_Default_Setting_Redo.rds")
...
t_full_fit_finished_LL_only <- lapply(X =1:length(t_full_fit_finished), FUN = Diagnostic_Plot_10, fit_Model_list = t_full_fit_finished, IFRvals = IFR_coefficients,
...
Age_groups <- readRDS("analysis/data/Code-generated-data/00_03_IFR_values_Brazeau.rds")$IFR_Age_gr
...
IFRs_Braz <- readxl::read_xlsx("../Bonus Files/WHO_report_12_comparison/inputs/Brazeau_et_al.xlsx")
...
IFR_df_full <- readRDS("analysis/data/Code-generated-data/00_03_IFR_matrix_coefficients_log_scale.rds") %>%
...
##########################
pdf("figures/39_figure_5_heatmaps_info.pdf", height = 5, width = 10)
cowplot::plot_grid(p_i, h, c_i, nrow = 1, rel_widths = c(1,2,1))#1,2.8,1))
dev.off()

tiff("figures/39_figure_5_heatmaps_info.tiff", height = 5, width = 10, units = "in", res = 150)
cowplot::plot_grid(p_i, h, c_i, nrow = 1, rel_widths = c(1,2,1))#1,2.8,1))
dev.off()
