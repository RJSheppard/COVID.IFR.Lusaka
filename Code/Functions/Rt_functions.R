...
# browser()
  ...
  country_plot <- function(vjust = -1.2) {
    ...
    g_tmp <- ggplot(sum_rt %>% filter(
      ...
      geom_ribbon(mapping = aes(x=date, ymin=Rt_min, ymax = Rt_max, group = iso), fill = "#8cbbca") +
      geom_ribbon(mapping = aes(x = date, ymin = Rt_q25, ymax = Rt_q75, group = iso, fill = "Rt")) +#, fill = "#3f8da7") +
      geom_ribbon(mapping = aes(x=date, ymin=Reff_min, ymax = Reff_max, group = iso), fill = "#96c4aa") +
      geom_ribbon(mapping = aes(x = date, ymin = Reff_q25, ymax = Reff_q75, group = iso, fill = "Reff")) + #, fill = "#48996b") +
    ...
    # if(Rt_only == F){
    #   # browser()
    #   g_tmp + geom_ribbon(mapping = aes(x=date, ymin=Reff_min, ymax = Reff_max, group = iso), fill = "#96c4aa") +
    #     geom_ribbon(mapping = aes(x = date, ymin = Reff_q25, ymax = Reff_q75, group = iso, fill = "Reff")) #, fill = "#48996b") +
    # }

    # if(Plot_R0 == T){
      # g_tmp + geom_hline(yintercept = sum_rt$R0_median[1], linetype = "dashed")
    # }
