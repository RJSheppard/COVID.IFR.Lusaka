...
WHO_estimate_region_age_year <- read_xlsx(path = "./inputs/WHO_estimates/WHO_COVID_Excess_Deaths_EstimatesByRegion.xlsx",sheet="Region by year, sex and age", skip = 10)
WHO_estimate_region_year <- read_xlsx("./inputs/WHO_estimates/WHO_COVID_Excess_Deaths_EstimatesByRegion.xlsx",sheet="Region by year and month", skip = 11)
WHO_estimate_ctry_age_year <- read_xlsx("./inputs/WHO_estimates/WHO_COVID_Excess_Deaths_EstimatesByCountry.xlsx",sheet="Country by year, sex and age", skip = 10)
WHO_estimate_ctry_year <- read_xlsx("./inputs/WHO_estimates/WHO_COVID_Excess_Deaths_EstimatesByCountry.xlsx",sheet="Country rate by year", skip = 8)

# UN population demographics
un_demog <- read_xlsx(path = "./inputs/unpopulation_dataportal_20220810111500.xlsx",sheet="Data",skip = 4)
...
un_for_iso <- read_xlsx("./inputs/WPP2022_GEN_F01_DEMOGRAPHIC_INDICATORS_COMPACT_REV1.xlsx",sheet="Estimates",skip = 15)
...
WHO_regions<-read.csv("./inputs/who-regions.csv")%>%
  rename(iso3=Code)

# Get Brazeau et al. estimates of COVID-19 IFR (including CIs)
brazeau <- read_xlsx("inputs/Brazeau_et_al.xlsx")
...
saveRDS(Complete_Demog, "Complete_Population_demog.rds")
...
Confirmed_deaths_WHO <- read.csv(file = "~/Downloads/WHO-COVID-19-global-data.csv", header = T)
...
IFR_vals_Braz <- readRDS("Brazeau_IFR_Post_predictive_CrIs.rds") %>%
...
############################################
# Save plot
tiff("Figure_1_WHO_Excess_Mortality_DVWI.tiff",height=10,width=10,unit="in",res=200)
