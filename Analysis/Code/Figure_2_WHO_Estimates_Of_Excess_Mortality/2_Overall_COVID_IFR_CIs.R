...
ifrdat <- readRDS("Analysis/Data/raw_data/ifrdat.RDS") # Available on request

ifrdat %>% filter(sero == "reg") %>% pull(bestestmod)
ifrdat %>% filter(sero == "reg") %>% pull(linear_predints)

# read in Richard's countries
cntry <- readr::read_tsv("Analysis/Data/raw_data/countries.txt", col_names = F)
cntry <- unlist(cntry)

# tidy up UN World Population Prospects
wpp <- readxl::read_excel("Analysis/Data/raw_data/wpp_un_agepopulations.xlsx")
...
Complete_Population_demog <- readRDS(file = "Analysis/Data/derived_data/Complete_Population_demog.rds") %>% rename(georegion = area,
                                                                                                                      ageband = age_cat,
                                                                                                                      popN = n) %>%
...
############################################
############################################
saveRDS(out, "Analysis/Data/derived_data/02_Brazeau_IFR_Post_predictive_CrIs.rds")
############################################
############################################
