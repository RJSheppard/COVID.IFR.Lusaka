...
WHO_estimate_region_age_year <- read_xlsx(path = "Analysis/Data/raw_data/WHO_COVID_Excess_Deaths_EstimatesByRegion.xlsx",sheet="Region by year, sex and age", skip = 10)
WHO_estimate_region_year <- read_xlsx("Analysis/Data/raw_data/WHO_COVID_Excess_Deaths_EstimatesByRegion.xlsx",sheet="Region by year and month", skip = 11)
WHO_estimate_ctry_age_year <- read_xlsx("Analysis/Data/raw_data/WHO_COVID_Excess_Deaths_EstimatesByCountry.xlsx",sheet="Country by year, sex and age", skip = 10)
WHO_estimate_ctry_year <- read_xlsx("Analysis/Data/raw_data/WHO_COVID_Excess_Deaths_EstimatesByCountry.xlsx",sheet="Country rate by year", skip = 8)

# UN population demographics
un_demog <- read_xlsx(path = "Analysis/Data/raw_data/unpopulation_dataportal_20220810111500.xlsx",sheet="Data",skip = 4)
...
un_for_iso <- read_xlsx("Analysis/Data/raw_data/WPP2022_GEN_F01_DEMOGRAPHIC_INDICATORS_COMPACT_REV1.xlsx",sheet="Estimates",skip = 15)
...
WHO_regions<-read.csv("Analysis/Data/raw_data/who-regions.csv")%>%
  rename(iso3=Code)

# Get Brazeau et al. estimates of COVID-19 IFR (including CIs)
brazeau <- read_xlsx("Analysis/Data/raw_data/Brazeau_et_al.xlsx") # Available on reasonable request
...
# saveRDS(Complete_Demog, "Analysis/Data/derived_data/02_Complete_Population_demog.rds")
saveRDS(Complete_Demog, "Analysis/Data/derived_data/Complete_Population_demog.rds")
...
Confirmed_deaths_WHO <- read.csv(file = "Analysis/Data/raw_data/WHO-COVID-19-global-data.csv", header = T)
...
IFR_vals_Braz <- readRDS("Analysis/Data/derived_data/Brazeau_IFR_Post_predictive_CrIs.rds") %>%
...
DVWI_plot<-ggplot(
  ...
  geom_vline(xintercept='Zambia',color="deepskyblue",lwd=4,alpha=0.1)+
  geom_point(aes(color=area_order), position = position_dodge(0.3), size =1.2)+
  geom_point(data = ann_text, aes(x = area_order, y = -0.02), color="lightgrey", shape = "*", inherit.aes = F, size = 7/.pt) +
  geom_text(data = ann_text2,label="*mean estimated excess deaths<0",vjust = 1,color="gray40",size=7/.pt,hjust=0)+
...
############################################
# Save plot
ggsave("Analysis/Figures/Figure_2_WHO_Excess_Mortality_DVWI.pdf",
...
IFR_vals_Braz %>% filter(area %in% c("Zambia",
                                     "Western Pacific",
                                     "Algeria","South Africa",
                                     "Europe","Americas",
                                     "Seychelles", "Mauritius", "Kenya", "Togo"))
